...
| Clerk 🟢 | `npx -y clerk` | `clerk auth login` *(auto-opens the popup, localhost callback; if the browser can't open it prints `Could not open your browser automatically. Open this URL to continue: <url>` — surface that. NOT device-code)* | `clerk whoami` | — *(no env-only CLI login — `clerk auth login` stores a token in the OS keychain; `CLERK_SECRET_KEY` is the app's server-side key and does NOT authenticate the CLI)* |
...
| Supabase 🟢 | `npx -y supabase` / `brew install supabase/tap/supabase` | `supabase login` *(DEFAULT: opens browser, CLI polls + stores the token automatically, prints `Finished supabase login.` — no paste. `--no-browser` prints a token URL then `Enter your access token:` to paste a PAT ⌨️; `--token`/`SUPABASE_ACCESS_TOKEN` skips it)* | `supabase projects list` | `SUPABASE_ACCESS_TOKEN` |
| Auth0 🔵⌨️ | `brew install auth0/auth0-cli/auth0` | `auth0 login` *(survey `How would you like to authenticate?` → `As a user` / `As a machine`, then `Press Enter`; match the user code)* | `auth0 tenants list` | — *(no env token; CI = machine login `auth0 login --client-id … --client-secret … --domain …`)* |
...
- **Supabase** *(only the `--no-browser` fallback gates)* — the default `supabase login` opens the browser and polls, no paste; `--no-browser` prints a token URL then `Enter your access token` (paste the PAT; or skip with `--token`/`SUPABASE_ACCESS_TOKEN`).
