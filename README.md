...
| [`agent-auth`](./skills/agent-auth) | Log a real human into a dev tool's CLI by auto-opening the OAuth browser popup and letting the login complete itself — no token copy-paste, no browser automation. Sanity + Netlify recipes plus a verified login reference for 20 dev-tool CLIs. |
